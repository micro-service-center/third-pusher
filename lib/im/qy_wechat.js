...
class QyWechat extends Platform {
  // 生成通知链接
  buildLink (resourceType, _resourceId, orgMap) {
    return `${this.opts.appHost}/corpId/${orgMap.refId}/${resourceType}/${_resourceId}/detail`
  }
  ...
  * pushLinkMsg (message, openids, orgMap) {
    ...
    // 获取需要推送的部门
    let agentid = getAgentid(corpInfo.agents, this.opts.appid[message.resourceType])
    ...
    let body = {
      ...
      textcard: {
        url: message.link,
        title: message.title,
        description: `${message.objectName}: ${message.resourceName}`
      }
    }
  ...
  * getCorpAccessToken (corpId, permanentCode, appType) {
    ...
    let corpAccessTokenKey = redis.corpAccessTokenKey(corpId, appType)
  ...
  * getSuiteAccessToken (appType) {
    let suiteInfo = yield [
      redis.get(redis.suiteTicketKey(appType)),
      redis.get(redis.suiteAccessTokenKey(appType))
    ]
    ...
    if (res.status >= 300 || !suiteAccessToken) {
      ilog.info({
        class: 'qywechat-suite-token-error',
        res: res.data,
        status: res.status,
        data: body
      })
    ...
    yield redis.setex(redis.suiteAccessTokenKey(appType), 30 * 60, suiteInfo[0] + suiteAccessToken)
